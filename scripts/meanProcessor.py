...
parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,description='Zabbix API python script to create a mean trigger for discovered CPU with SNMP.', epilog="""
...
hosts=aux
key = 'system.cpu.util[hrProcessorLoad'
hosts,items = selectHosts(hosts)
print "End of research"
for i in range(len(hosts)):
    print "About " + hosts[i]
    if (zapi.trigger.get(filter={'host': hosts[i]},search={'description':'#PythonScriptCPU:'}) == []):
        triggers = zapi.trigger.get(filter={'host': hosts[i]},search={'description':'High CPU utilization'})
        final = "("
