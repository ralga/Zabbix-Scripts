...
parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,description='Script that list enabled host that have neither Zabbix agent nor SNMP monitoring', epilog="""
...
for host in hosts:
    if(host['status']=='0'):
        hasInfo= False
        isAgent = host['available'] == 1
        if(not isAgent):
            interfaces = zapi.hostinterface.get(filter={'hostid':host['hostid']})
            for interface in interfaces :
                if(interface['type']=='2'):
                    hasInfo = True
                    break
        if ((not hasInfo)and(not isAgent)):
            print (host['name'])
